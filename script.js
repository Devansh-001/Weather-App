...
const getFutureData = async (long, lat) => {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${long}&daily=temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=Asia/Tokyo`;
    ...
    data.daily.time.forEach((time, i) => {
        const date = new Date(time).toDateString(); // Convert ISO date string to Date
        const maxTemp = dailyForecasts.temperature_2m_max[i];
        const minTemp = dailyForecasts.temperature_2m_min[i];
        const precipitation = dailyForecasts.precipitation_sum[i];

        console.log(`Date: ${date}, Max Temp: ${maxTemp}°C, Min Temp: ${minTemp}°C, Precipitation: ${precipitation}mm`);
    });
}
